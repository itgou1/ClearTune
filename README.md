...
当前源码版本：`1.3.4` · [本次更新说明](release-notes/v1.3.4.md) · [已发布安装包](https://github.com/itgou1/ClearTune/releases)
...
专辑、艺术家和歌单详情分别保留所属页面的数据与加载任务，返回手势预览不会覆盖当前页面内容；取消返回后保留当前歌曲列表和滚动位置，现有转场动画保持不变。
...
本版沿用 1.3.3 的账号数据库与离线音频目录，不要求重新同步曲库或重新下载已有歌曲。封面临时缓存使用新的账号隔离标识，首次显示可能需要重新加载；已有离线歌曲会按下载网络设置后台补齐封面，封面失败不会使音频下载失效。

发布前已完成 115 项单元测试和 64 项模拟器设备测试，全部通过；静态检查无错误。测试范围与真机验收边界见 [1.3.4 测试记录](release-notes/v1.3.4-testing.md)。
