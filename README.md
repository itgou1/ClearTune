...
当前源码版本：`1.3.5` · [本次更新说明](release-notes/v1.3.5.md) · [已发布安装包](https://github.com/itgou1/ClearTune/releases)

## 真实界面

以下为 1.3.3 源码构建的 Debug 应用在 Android 模拟器中的直接截图，展示本版沿用的首页与“我的”页面布局；1.3.4 的新歌标记、批量选择和详情封面变化请参见更新说明。截图使用本地演示服务器、虚构曲目和原创插画封面，不连接真实用户账号，也不对截图中的控件或布局进行重绘。
...
本版沿用现有账号数据库、设置、队列与离线音频目录，不要求重新下载已有歌曲。从 1.3.3 升级时仍会应用 1.3.4 的封面缓存改进：首次显示可能需要重新加载，已有离线歌曲按下载网络设置补齐封面，封面失败不影响音频播放。

本次已完成离线冷启动、保留数据覆盖安装和系统媒体按键等针对性模拟器验收，发布流程对最终版本执行单元测试、静态检查和签名构建。测试范围与真机验收边界见 [1.3.5 更新说明](release-notes/v1.3.5.md)。
